...
O(n) 이라면 연락처에 100명이 있다고 할때 원하는 이름을 검색하고 싶다면 각 이름을 모두 확인해야 하기 때문에 
  약 100번의 연산이 필요하다 이후, 갯수가 1000, 10000명으로 늘어나면 똑같이 연산수도 1000, 10000번으로 늘어난다. 
반면 O(logN) 이라면 
이 정렬되어있는 데이터에서 특정 이름을 검색하기 위해 이진 검색을 사용하기에 연산수를 절반으로 줄여나가며 더 빠르게 검색할 수 있다. 
