...
이 원칙들을 준수함으로써 소프트웨어는 모듈화, 유지보수성, 테스트 용이성을 갖춘 구조로 설계된다.
...
Stream API의 `map()`과 `flatMap()`은 데이터 변환과 처리에 사용되는 중요한 메서드이다. 두 메서드 모두 스트림의 요소를 변환하지만, 처리 방식과 반환 결과에서 큰 차이가 있다.
